...
void MPC::updateAll(cVector3d a_pos, cVector3d a_euler, double a_xErr, double a_zErr, double a_rollErr, double a_yawErr, double a_X1, double a_Y1, double a_X2, double a_Y2) {

	// use controller update function to fill state vector
	LQRController::updateAll(a_pos, a_euler, a_xErr, a_zErr,a_rollErr, a_yawErr);
	
	// pass state to CVX params
	//for (int i = 0; i < state.size(); i++) {
	//	params.x_0[i] = state[i];
	//}
	params.x_0[0] = state[0];	// xErr
	params.x_0[1] = state[3];	// yErr
	params.x_0[2] = state[6];	// rollErr
	params.x_0[3] = state[9];	// yawErr
	params.x_0[4] = state[1];	// u
	params.x_0[5] = state[4];	// w
	params.x_0[6] = state[7];   // p
	params.x_0[7] = state[10];	// r



	params.u_int_start[0] = a_X1;
	params.u_int_start[1] = a_Y1;
	params.u_int_start[2] = a_X2;
	params.u_int_start[3] = a_Y2;
...
void MPC::updateSystem() {
	...
	for (int i = 0; i < Q.size(); i++) {
		params.Q[i] = Q[i]/100.0;
		params.Q_final[i] = params.Q[i];
	}
...
void MPC::setConstraints() {
	for (int i = 0; i < nInputs; i++) {
		params.u_int_max[i] = 3.0;
		params.u_int_min[i] = -3.0;
